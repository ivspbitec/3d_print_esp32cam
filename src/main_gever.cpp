...
const char* apPassword = "987654321A";
  ...
  // Подключение к сохраненной сети Wi-Fi
        ...
        if (WiFi.status() == WL_CONNECTED) {          
          ...
          float temperature = (float)temperatureRead() / 100.0;
            ...
            lcdPrint("Wifi Connected. SSID: %s, IP: %s, RSSI: %d dBm, Temperature: %.2f °C",
...
void setup() {
    ...
    wifiInit();
        ...
        if (isWifiConnect!=1){
            ...
            server.on("/save", HTTP_GET, handleSettingsSave);
...
void handleLEDOn() {
    for(int i=0; i<strip.numPixels(); i++) {
        strip.setPixelColor(i, strip.Color(255, 255, 255)); // Устанавливаем белый цвет
    }
    strip.show();
